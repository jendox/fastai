...
## Установка и настройка библиотеки для работы с Gotenberg

Библиотека [Gotenberg API](https://github.com/devmanorg/gotenberg-api) упрощает работу с endpoint API для генерации скриншотов [gotenberg.dev](https://gotenberg.dev/docs/routes#screenshots-route), предоставляя тонкую обёртку над веб API и библиотекой [HTTPX](https://www.python-httpx.org/).
...
Установить из публичного GitHub-репозитория:
```bash
$ uv add "gotenberg-api @ git+https://github.com/devmanorg/gotenberg-api.git"
```
Установить из приватного GitLab-репозитория (при наличии доступа):
```bash
$ uv add "gotenberg-api @ git+https://gitlab.dvmn.org/dvmn/courses/fastapi/gotenberg-api.git"
```

### Использование

Библиотека использует асинхронный клиент httpx.AsyncClient.

**Обязательные настройки**

httpx.AsyncClient.base_url — базовый адрес Gotenberg API (например, http://127.0.0.1:3000).

ScreenshotHTMLRequest.width — ширина скриншота в пикселях.

ScreenshotHTMLRequest.format — формат скриншота: jpeg, png или webp (по умолчанию — jpeg).

ScreenshotHTMLRequest.wait_delay — время ожидания завершения анимаций на HTML-странице (по умолчанию — 2 секунды).

> **⚠️ ВАЖНО: время ожидания завершения анимаций должно быть меньше таймаута асинхронного клиента, иначе библиотека всегда будет возвращать TimeoutError.
